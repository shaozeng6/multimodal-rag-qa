...
> ⚠️ **P0 强制改密**: 登录后系统会强制要求先修改初始密码, 否则无法进入系统(路由拦截)。
> 存量升级时, 已有 admin 账号也会被标记一次强制改密。改密成功后标记清除, 不再强制。
> 生产环境还应替换 `.env` 中的 `JWT_SECRET`(缺省/占位值会导致启动拒绝, 过短会告警)。
...
- **配置范围约束**:仅收录实际需要调参且已接线的运行参数——检索召回(topK)、评估阈值、
...
- **文件列表 = 已入库文档 + 进行中任务**:上传完成后文件即出现在文件列表中, 进行中/失败/已取消的任务行内嵌**进度条 + 管道步骤条**(解析:上传→OCR→分片 ‖ 索引:描述→向量化→入库, 分阶段显示完成/进行中/待执行)与阶段文案, 无需单独的"入库任务"页。
- **两段式管道(可复用)**:解析(OCR/分片)产物 **chunks.json 落盘**, 索引(描述/向量化/入库)从落盘读取——**换嵌入模型/重跑索引不必重新 OCR/分片**; 失败重试自动只重跑失败段(索引失败重试跳过解析)。
- **手动控制(分步)**:上传时勾选「手动控制(分步)」, 上传后**停在列表不做任何处理**, 分别点「**解析**」(OCR/分片) → 完成后点「**入库**」(描述/向量化/写入) 两段手动执行; 解析完成停在「待入库」状态。
- **行内操作**:进行中任务可**暂停**(阶段边界+向量化循环内逐条)、**继续**、**取消**; 失败/已取消任务可**删除**(清记录+中间产物)或**重试**(复用原 job_id, 只重跑失败段); 每行支持查看**任务日志**。
- **先不处理**:上传时勾选「上传后先不处理」, 任务停在首阶段(如先只上传、暂不 OCR), 后续在列表中点击「继续」。
...
管理工作台第三个 Tab, 提供用户/角色体系:

- **创建用户**: 新建用户(普通用户/管理员), 新用户首登强制改密。
- **改角色 / 启禁用**: 角色下拉就地修改; 禁用后该账号无法登录(保留会话/消息历史)。
- **重置密码**: 管理员设新密码, 重置后该用户首登强制改密。
- **删除用户**: 删除账号与关联会话/消息。
- **自我保护守卫**: 不能修改自己的角色、禁用或删除自己; 至少保留一个管理员(后端强制)。
- 登录校验: 禁用账号返回 403「账号已被禁用」。

## 审核队列(管理员)

管理工作台第四个 Tab, 人工审批的补充:

- **按角色分流**: 管理员会话的低分回答仍走**即时审批中断**(原流程); 普通用户低分回答**不中断**、
  直接交付, 自动标记待审进入本队列。
- **处理**: 查看待审回答(问题/回答/评分) → 通过 / 忽略(清除标记并记录处理动作)。
- 普通用户回答入库仍受记忆质量线约束(低分不写入跨会话记忆)。
