...
跨会话的过往高质量问答以「问题 + 回答」对的形式写入 `t_context`;检索时对问题与回答双路召回(问题路为主、回答路兜底),命中并入统一召回结果并附带来源标签。

**时效与淘汰**(2026-08): 记忆库按 `memory.ttl_days`(默认 180 天)硬过滤过时条目、不再作为回答依据;命中按年龄**软衰减**重排(≤30 天 1.0 / ≤90 天 0.5 / 更老 0.25),近期答案优先进上下文;每用户上限 `memory.max_per_user`(默认 500 条),超限惰性淘汰最旧;后台每日清理过期条目回收存储。参数在「系统设置 → 记忆」分组即时调整。
...
| | `MAX_PDF_UPLOAD_MB` | PDF 上传大小上限(默认 50MB,超限返回 413) |
...
- **分组**:按入库 / 检索 / 记忆 / 评估 / 上下文 / 图片上限分组,组内按逻辑子域(分片 / 向量化 / 候选数 / 融合 / 记忆时效 / 阈值…)呈现,支持按组保存与恢复默认值。
- **生效方式**:全部即时生效(保存即写入并热加载)。
- **配置范围**:仅收录实际需要调参且已接线的运行参数,包括检索召回(topK)、评估阈值、分片字符数、上下文窗口、图片上限、记忆有效期与条数上限。模型型号 / 温度 / 维度、OCR 线程 / DPI、限流等基础设施配置仍归属 `.env`(重启级),不在设置页内。
...
- **上传限制与去重**:PDF 大小上限 `MAX_PDF_UPLOAD_MB`(默认 50MB,流式落盘,超限返回 413);按内容 md5 查重,同文件重复上传返回 409,避免重复文档与重复向量。
...
| `interrupt` | 命中人工审批(携带草稿答案、评分与 session_id,前端据此恢复审批弹窗) |
