...
- **caption 桥接**:图片输入经视觉语言(VL)模型生成描述,作为文本查询检索知识库,实现「以图找文」。
- **图文一致性**:检测用户文字与图片内容是否一致(`related` / `unrelated` / `contradictory` 三态);发生矛盾时以图片为准,单独使用 caption 检索,避免错误描述污染结果。
- **记忆检索**:跨会话的过往高质量问答写入 `t_context`,检索时并入统一召回结果并附带来源标签。
